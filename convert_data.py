import os
from enum import IntEnum

from asn1.GSMA import SGP_22
from asn1.TCA import eUICC_Profile_Package
from converter.converter import decode_encode_asn
from helper.test_data import TestData
from helper.util import print_done


class options(IntEnum):
    parse_any_data_console_output = 1
    parse_store_meta_data_console_output = 2
    parse_profile_element_data_console_output = 3


def parse_or_update_any_data(base_data, input_format='der', output_format='asn1', print_inp=None, re_parse_op=None,
                             asn1_element=None):
    if isinstance(base_data, list):
        parsed_data_list = []
        for data in base_data:
            parsed_data_list.append(
                parse_or_update_any_data(base_data=data, input_format=input_format, output_format=output_format,
                                         print_inp=print_inp, re_parse_op=re_parse_op, asn1_element=asn1_element))
        return parsed_data_list
    if os.path.isfile(base_data):
        # file is provided
        try:
            with open(base_data, 'r') as the_file:
                resp = ''.join(the_file.readlines())
        except UnicodeDecodeError:
            # Binary File
            with open(base_data, 'rb') as the_file:
                resp = the_file.read()
        if os.path.splitext(base_data)[1] == ".asn1":
            input_format = 'asn1'
            if output_format in ['asn1']:
                output_format = 'der'
        base_data = resp

    # Set Default Values if nothing is set
    if print_inp is None: print_inp = False
    if re_parse_op is None: re_parse_op = False
    re_parsed_data = None
    # parse Data
    parsed_data = decode_encode_asn(input_data=base_data, parse_only=True,
                                    input_format=input_format, output_format=output_format,
                                    asn1_element=asn1_element)
    if re_parse_op:
        re_parsed_data = decode_encode_asn(input_data=parsed_data, parse_only=True,
                                           input_format=output_format, output_format=input_format,
                                           asn1_element=asn1_element)
    if print_inp:
        print('\nInput Data is: ')
        print(base_data)
    print('\nOutPut Data is: ')
    print(parsed_data)
    if re_parse_op:
        print('\nRe-parsed Data is: ')
        print(re_parsed_data)
    return parsed_data


def test_data(option):
    print_ip = False
    parse_op = False
    asn1_element = None

    if option == options.parse_store_meta_data_console_output:
        asn1_element = SGP_22.RSPDefinitions.StoreMetadataRequest
        #
        data_pool = [
            "",
            r"D:\Other\Github_Self\euiccSpecs\SGP.23 RSP Test Specification\SampleData\FULL_METADATA.asn1",
            r"D:\Other\Github_Self\euiccSpecs\SGP.23 RSP Test Specification\SampleData\FULL_METADATA_wo_icon.asn1",
            r"D:\Other\Github_Self\euiccSpecs\SGP.23 RSP Test Specification\SampleData\FULL_METADATA.hex",
            r"D:\Other\Github_Self\euiccSpecs\SGP.23 RSP Test Specification\SampleData\FULL_METADATA_wo_icon.hex",
            r"D:\Other\Github_Self\euiccSpecs\SGP.23 RSP Test Specification\SampleData\FULL_METADATA.base64",
            r"D:\Other\Github_Self\euiccSpecs\SGP.23 RSP Test Specification\SampleData\FULL_METADATA_wo_icon.base64",
            r"D:\Other\Github_Self\euiccSpecs\SGP.23 RSP Test Specification\SampleData\FULL_METADATA_Generated.asn1",
            r"D:\Other\Github_Self\euiccSpecs\SGP.23 RSP Test Specification\SampleData\FULL_METADATA_wo_icon_Generated.asn1",
        ]
        for data in data_pool:
            parse_or_update_any_data(data, print_inp=print_ip, re_parse_op=parse_op, asn1_element=asn1_element)

    if option == options.parse_profile_element_data_console_output:
        asn1_element = eUICC_Profile_Package.PEDefinitions.ProfileElement
        #
        data_pool = [
            r'D:\Other\Github_Self\euiccSpecs\SGP.23 RSP Test Specification\SampleData\PROFILE_OPERATIONAL1.asn1',
            "",
        ]
        for data in data_pool:
            parse_or_update_any_data(data, print_inp=print_ip, re_parse_op=parse_op, asn1_element=asn1_element)

    if option == options.parse_any_data_console_output:
        test_data_pool = [
            TestData
            (data="""{
            profilePolicyRules {ppr2}
        }""",
             input_format='asn1', output_format='der', asn1_element=SGP_22.RSPDefinitions.UpdateMetadataRequest
             ),
            TestData(data='bf2a0499020520', asn1_element=SGP_22.RSPDefinitions.UpdateMetadataRequest),
        ]
        for test_data in test_data_pool:
            parse_or_update_any_data(test_data.data, print_inp=print_ip, re_parse_op=parse_op,
                                     asn1_element=test_data.asn1_element, input_format=test_data.input_format,
                                     output_format=test_data.output_format)


def main():
    """

    :return:
    """
    global log
    test_data_options = [
        options.parse_any_data_console_output,
        options.parse_store_meta_data_console_output,
        options.parse_profile_element_data_console_output,
    ]
    for test_data_option in test_data_options:
        test_data(test_data_option)
    print_done()


if __name__ == '__main__':
    main()
